...
class LevelIMG:
    ...
    def drawIMG(self):
        ...
        d.text((920-XP_w,250-XP_ht),XP,fill=clr_bar,font=font)
        d.text((920-LVL_w,10),LVL,fill=(255,255,255),font=font)
        d.text((920-RNK_w-LVL_w-20,10),RNK,fill=clr_bar,font=font)
       
        if di == 100:
            percentVal1 = self.exp - ((self.lvl-1)*di)
            percent = (percentVal1/di)*934
            d.rectangle([0,272,percent,282],fill=clr_bar)
        if di != 100:
            percentVal1 = self.exp - ((self.lvl-1)*di)
            percent = (percentVal1/di)*100
            bar_to_be_filled = (percent/100)*934
            d.rectangle([0,272,bar_to_be_filled,282],fill=clr_bar)
