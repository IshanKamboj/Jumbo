...
class InfoCogs(commands.Cog):
    ...
    async def _avatar(self, ctx, user: discord.Member = None):
        if user == None:
            user = ctx.author
        em = discord.Embed(
            description=f"[PNG](https://cdn.discordapp.com/avatars/{user.id}/{user.avatar}.png?size=1024) | [JPEG](https://cdn.discordapp.com/avatars/{user.id}/{user.avatar}.jpeg?size=1024) | [WEBP](https://cdn.discordapp.com/avatars/{user.id}/{user.avatar}.webp?size=1024)", color=discord.Color.random())
